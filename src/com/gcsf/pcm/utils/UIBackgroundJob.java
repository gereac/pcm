...
public abstract class UIBackgroundJob extends Job {
   ...
   * 
   * @param control
   *          Used to cancel the UI-Job in case the given Control is disposed at
   *          that time.
   */
  ...
  private synchronized void synchronizedSafeRunInBackground(
  ...
  private synchronized void synchronizedSafeRunInUI(
      final IProgressMonitor monitor) {
    SafeRunner.run(new LoggingSafeRunnable() {
      public void run() throws Exception {
        runInUI(monitor);
