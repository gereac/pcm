...
import com.gcsf.pcm.utils.JobRunner;
import com.gcsf.pcm.utils.UIUtils;
 ...
 * @author bpasero
 */
class ContactsSearchBar extends Composite {
   ...
   * 
   * @param viewSite
   *          A link to the ViewSite this Bar is in.
   * @param parent
   *          parent <code>Composite</code>
   * @param viewer
   *          The Viewer this Filter is working for.
   * @param patternFilter
   *          The PatternFilter to use.
   */
  ContactsSearchBar(IViewSite viewSite, Composite parent, TreeViewer viewer) {
  ...
  private void createClearText(Composite parent) {
    ...
    fFilterToolBar.getControl().setLayoutData(
    ...
    clearTextAction.setImageDescriptor(Activator
        .getImageDescriptor("icons/clear/clear.gif")); //$NON-NLS-1$
  ...
  private void createControl(Composite parent) {
    ...
    fFilterComposite.setLayoutData(new GridData(SWT.FILL, SWT.BEGINNING, true,
        false));
    fFilterComposite.setBackground(parent.getBackground());

    /* Set height as required */
    if (fFilterToolBar != null) {
      int prefContainerHeight = fFilterComposite.computeSize(SWT.DEFAULT,
          SWT.DEFAULT).y;
      int prefToolbarHeight = fFilterToolBar.getControl().computeSize(
  ...
  private void createFilterTarget(Composite parent) {
    ...
    IAction filterTargetAction = new Action("", IAction.AS_DROP_DOWN_MENU) { //$NON-NLS-1$
      @Override
      public void run() {
        UIUtils.positionDropDownMenu(this, filterTargetManager);
      }
    ...
    // searchName
    // .setSelection(BookMarkFilter.SearchTarget.NAME == fPatternFilter
    // .getSearchTarget());
    // searchName.addSelectionListener(new SelectionAdapter() {
    // @Override
    // public void widgetSelected(SelectionEvent e) {
    // if (searchName.getSelection()
    // && fPatternFilter.getSearchTarget() != BookMarkFilter.SearchTarget.NAME)
    ...
    // searchLink
    // .setSelection(BookMarkFilter.SearchTarget.LINK == fPatternFilter
    // .getSearchTarget());
    // searchLink.addSelectionListener(new SelectionAdapter() {
    // @Override
    // public void widgetSelected(SelectionEvent e) {
    // if (searchLink.getSelection()
    // && fPatternFilter.getSearchTarget() != BookMarkFilter.SearchTarget.LINK)
    ...
    // });
    //
    // filterTargetAction.setImageDescriptor(OwlUI
    //        .getImageDescriptor("icons/etool16/find.gif")); //$NON-NLS-1$
    // filterTargetManager.add(filterTargetAction);
    //
    // filterTargetManager.createControl(parent);
    // filterTargetManager.getControl().setLayoutData(
    // new GridData(SWT.BEGINNING, SWT.CENTER, false, true));
  }
  ...
  private void createFilterText(Composite parent) {
    ...
    fFilterText.addFocusListener(new FocusAdapter() {
      ...
      public void focusGained(FocusEvent e) {
        ...
        fViewSite.getActionBars()
            .getGlobalActionHandler(ActionFactory.CUT.getId()).setEnabled(true);
        fViewSite.getActionBars()
      ...
      public void keyTraversed(TraverseEvent e) {
        if (e.detail == SWT.TRAVERSE_RETURN) {
          ...
          if (fViewer.getTree().getItemCount() > 0) {
            ...
            boolean textChanged = !fInitialText.equals(fFilterText.getText()
                .trim());

            /* Select the first matching Item */
            if (hasFocus && textChanged
                && fFilterText.getText().trim().length() > 0) {
        ...
        try {
          ...
          if (text.length() == 0) {
            ...
            if (fSelectedElements != null)
              fViewer.setSelection(new StructuredSelection(fSelectedElements),
                  true);
  ...
  void textChanged() {
    fRefreshJob.cancel();
    fRefreshJob
        .schedule(fFilterText.getText().length() != 0 ? FILTER_DELAY : 0);
  }
